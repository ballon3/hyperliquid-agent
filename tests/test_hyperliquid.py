...
def test_get_balance(client):
    ...
    client.exchange.fetch_balance = MagicMock(
        return_value={"total": {"BTC": 0.5, "ETH": 2.0, "USDC": 10000}}
    )

    balance = client.get_balance()

    assert balance == {"BTC": 0.5, "ETH": 2.0, "USDC": 10000}, (
        "Balance does not match expected result"
    )
...
def test_get_market_data(client):
    ...
    assert market_data == [1700000000, 40000, 40500, 39500, 40200, 100], (
        "Market data does not match expected result"
    )
    client.exchange.fetch_ohlcv.assert_called_once_with(
        "BTC/USDC", timeframe="1m", limit=1
    )
